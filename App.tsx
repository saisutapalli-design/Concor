...
const App: React.FC = () => {
  ...
  const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Landing and Public pages handle their own headers
    ...
    return (
      <div className="min-h-screen flex flex-col bg-white selection:bg-[#0096D6]/30">
        <header className="sticky top-0 z-[100] bg-white/95 backdrop-blur-md border-b border-slate-100 px-8 h-20 flex items-center justify-between shrink-0 shadow-sm">
          ...
          <div className="flex items-center gap-8">
            ...
            <Link to="/profile" className="flex items-center gap-4 pl-2 group">
              ...
              <div className="h-11 w-11 bg-slate-50 rounded-2xl flex items-center justify-center border border-slate-100 overflow-hidden transition-all group-hover:border-[#0096D6]/50 group-hover:bg-blue-50 group-active:scale-90">
        ...
        <footer className="bg-slate-950 pt-16 pb-12 px-8 border-t border-white/5 shrink-0 relative overflow-hidden text-center">
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.03] scale-[2.5] pointer-events-none brightness-0 invert flex justify-center h-20">{CONCOR_LOGO}</div>
          <div className="max-w-7xl mx-auto flex flex-col items-center gap-8 relative z-10">
            <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-3 text-[11px] font-black text-slate-500">
              <Link to="/dashboard" className="hover:text-[#0096D6] transition-colors uppercase tracking-[0.3em]">Dashboard</Link>
              <Link to="/profile" className="hover:text-[#0096D6] transition-colors uppercase tracking-[0.3em]">Credentials</Link>
              <Link to="/settings" className="hover:text-[#0096D6] transition-colors uppercase tracking-[0.3em]">Security</Link>
              <div className="h-4 w-px bg-white/10 hidden md:block"></div>
              <span className="uppercase tracking-[0.3em] text-slate-600">SECURE NODE: {location.pathname.replace('/', '').toUpperCase() || 'ROOT'}</span>
            </div>
            <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.3em] leading-relaxed max-w-lg">Copyright © 2026 Container Corporation of India Ltd.<br/>Authorized access only. All operational events are logged.</p>
          </div>
        </footer>
      </div>
    );
